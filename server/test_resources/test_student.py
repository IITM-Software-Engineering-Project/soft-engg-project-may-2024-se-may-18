...
def setup_test_data():
    ...
    try:
        ...
        user = User(id=1, username="testuser", role="student", last_login=datetime.now(
        ), email="test@example.com", password="testpass")
        test_db.add(user)
        test_db.commit()
        # Create and add test data
        course = Course(id=1, title="Test Course",
                        description="Test Description", total_modules=10, price=100.0)
        test_db.add(course)
        test_db.commit()

        module = Module(title="Test Module", total_lectures=5, total_assignments=3,
                        course_id=course.id, description="Test Description")
        test_db.add(module)
        test_db.commit()

        enrollment = CourseEnrollment(
            student_id=user.id, course_id=course.id, enrollment_date=datetime.now())
        test_db.add(enrollment)
        test_db.commit()

        assignment = Assignment(
            module_id=module.id, title="Test Assignment", type="homework", due_date=datetime.now())
        test_db.add(assignment)
        test_db.commit()

        exam = Exam(course_id=course.id, student_id=user.id,
                    exam_id=1, marks=90.0)
        test_db.add(exam)
        test_db.commit()
...
def test_get_student_course_overview_success(setup_test_data):
    # Test successful response
    response = client.get(
        "/student/enrolled_course/student-overview?course_id=1&student_id=1")
...
def test_get_student_course_overview_invalid_course_id(setup_test_data):
    # Test with invalid course_id
    response = client.get(
        "/student/enrolled_course/student-overview?course_id=999&student_id=1")
    assert response.status_code == 404
    assert response.json() == {"detail": "Enrollment not found"}


def test_get_student_course_overview_invalid_student_id(setup_test_data):
    # Test with invalid student_id
    response = client.get(
        "/student/enrolled_course/student-overview?course_id=1&student_id=999")
    assert response.status_code == 404
    assert response.json() == {"detail": "Enrollment not found"}


def test_get_student_course_overview_invalid_ids(setup_test_data):
    # Test with both invalid course_id and student_id
    response = client.get(
        "/student/enrolled_course/student-overview?course_id=999&student_id=999")
    assert response.status_code == 404
    assert response.json() == {"detail": "Enrollment not found"}


def test_get_module_details_success(setup_test_data):
    # Test successful response
...
def test_get_module_details_invalid_course_id(setup_test_data):
    # Test with invalid course_id
    response = client.get("/student/module-details/?course_id=999&module_id=1")
    assert response.status_code == 404
    assert response.json() == {"detail": "Module not found"}


def test_get_module_details_invalid_module_id(setup_test_data):
    # Test with invalid module_id
    response = client.get("/student/module-details/?course_id=1&module_id=999")
    assert response.status_code == 404
    assert response.json() == {"detail": "Module not found"}


def test_get_module_details_invalid_ids(setup_test_data):
    # Test with both invalid course_id and module_id
    response = client.get(
        "/student/module-detailsmodule-details/?course_id=999&module_id=999")
    assert response.status_code == 404
    assert response.json() == {"detail": "Module not found"}
...
def test_enroll_student_success(setup_test_data):
    response = client.post(
        "/student/enroll", json={"student_id": 1, "course_id": 2})
    assert response.status_code == 200
...
def test_enroll_student_success(setup_test_data):
    course = Course(id=2, title="Test Course 2",
                    description="Test Description 2", total_modules=10, price=100.0)
...
def test_enroll_student_invalid_student_id(setup_test_data):
    response = client.post(
        "/student/enroll", json={"student_id": 999, "course_id": 1})
    assert response.status_code == 404
...
def test_enroll_student_invalid_course_id(setup_test_data):
    response = client.post(
        "/student/enroll", json={"student_id": 1, "course_id": 999})
    assert response.status_code == 404
    assert response.json() == {"detail": "Student or Course not found"}
...
def test_enroll_student_invalid_ids(setup_test_data):  # Not there
    response = client.post(
        "/student/enroll", json={"student_id": 999, "course_id": 999})
    assert response.status_code == 404
    assert response.json() == {"detail": "Student or Course not found"}
...
def test_enroll_student_already_enrolled(setup_test_data):
    response = client.post(
        "/student/enroll", json={"student_id": 1, "course_id": 1})
    assert response.status_code == 400  # Assuming a conflict status code
    assert response.json() == {
        "detail": "Student is already enrolled in the course"}
